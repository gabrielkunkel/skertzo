...
describe("filter filter", function() {
  ...
  // Using the filter filter with a string matches anything that includes it vs. exact match
  ...
  // The filter filter will look throughout any collection for matches
  ...
  it('filter at this point filters numbers like strings', function() {
    //it will include Mary even though Mary doesn't have '42', but '423'
    var fn = parse('arr | filter:42');
    expect(fn({arr: [
      {name: 'Mary', age: 423},
      {name: 'John', age: 43},
  ...
  // If you want any of the properties in an object to match a value
  // Wildcard will basically catch the contents of any property, not just on the same level
  ...
  // wildcards are written as if the '$' was the properrty.
  ...
  it('filters with a nested wildcard property', function() {
    var fn = parse('arr | filter:{$: {$: "o"}}');
    expect(fn({arr: [
      {name: {first: 'Joe'}, role: 'admin'}, // take this one
      {name: {first: 'Jane'}, role: 'moderator'}, // skip this one
      {name: {first: 'Mary'}, role: 'admin'} // "control object"
    ]})).toEqual([
      {name: {first: 'Joe'}, role: 'admin'}
    ]);
  });
  ...
  // TODO: Try to create my own filter comparator, for example it could keep all names that begin with the first two letters of a given name.
